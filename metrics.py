...
def get_entities_ask_ubuntu(tags):
    entity_tags = {'SoftwareName', 'Printer', 'UbuntuVersion'}

    def collect_next_entity(tag_list, idx):
        while idx < len(tag_list) and tag_list[idx] not in entity_tags:
            idx += 1
        if idx < len(tag_list):
            start_idx = idx
            start_tag = tag_list[idx]
            idx += 1
            if idx < len(tag_list):
                curr_tag = tag_list[idx]
            else:
                return (start_tag, start_idx, idx), idx
            while curr_tag is not None and curr_tag in entity_tags and start_tag == curr_tag:
                idx += 1
                if idx < len(tag_list):
                    curr_tag = tag_list[idx]
                else:
                    curr_tag = None
            end_idx = idx
            return (start_tag, start_idx, end_idx), idx
        else:
            return None, idx
    ...
    i = 0
    while i < len(tags):
        next_ent, i = collect_next_entity(tags, i)
        if next_ent is not None:
            entities.add(next_ent)
    return entities


def ask_ubuntu_f1_score(prediction, ground_truth):
    pred_ents = get_entities_ask_ubuntu(prediction)
    gt_ents = get_entities_ask_ubuntu(ground_truth)

    intersect = len(pred_ents & gt_ents)

    if intersect == 0:
        return 0

    precision = 1.0 * intersect / len(pred_ents)
    recall = 1.0 * intersect / len(gt_ents)

    f1 = (2 * precision * recall) / (precision + recall)
    return f1


def get_entities_conll(tags):
    print(tags)
    entity_tags = {'I-PER', 'I-LOC', 'I-MISC', 'I-ORG', 'B-PER', 'B-LOC', 'B-MISC', 'B-ORG'}
    ...
    def collect_next_entity(tag_list, idx):
        ...
        if idx < len(tag_list):
            ...
            if idx < len(tag_list):
                ...
                return (start_tag.split('-')[1], start_idx, idx), idx
            while curr_tag is not None and \
                    ((curr_tag[0] == 'I' and curr_tag == start_tag) or
                        (start_tag[0] == 'B' and curr_tag[0] == 'I' and start_tag.split('-')[1] == curr_tag.split('-')[1])):
                idx += 1
                if idx < len(tag_list):
                    curr_tag = tag_list[idx]
                else:
                    curr_tag = None
            end_idx = idx
            return (start_tag.split('-')[1], start_idx, end_idx), idx
        else:
            return None, idx
...
def conll_f1_score(prediction, ground_truth):
    pred_ents = get_entities_conll(prediction)
    gt_ents = get_entities_conll(ground_truth)
...
def computeConllF1(outputs, targets):
    return sum([conll_f1_score(o, t) for o, t in zip(outputs, targets)]) / len(outputs) * 100
...
def compute_metrics(data, rouge=False, bleu=False, corpus_f1=False, logical_form=False, dialogue=False):
    ...
    conllF1 = computeConllF1(greedy, answer)
    # nf1 = computeF1(norm_greedy, norm_answer)
    # nem = computeEM(norm_greedy, norm_answer)
    metric_keys.extend(['conllF1'])
    metric_values.extend([conllF1])
    # if corpus_f1:
